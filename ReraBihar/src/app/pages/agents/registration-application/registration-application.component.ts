...
@Component({
    selector: 'app-registration-application',
    templateUrl: './registration-application.component.html',
    styleUrls: ['./registration-application.component.css']
})
export class RegistrationApplicationComponent implements OnInit {
    formDetails: any;
    stepList: any = [];
    stepListShow: any = [];
    backupDetails: any;
    ...
    aadhaarData: any;
    aadhaarData1: any = {
        "success": true, "status": 200, "message": "OTP verification was successful.", "response": {
            "status": 200, "body": {
                "data": {
                    "client_id": "aadhaar_v2_UYbbVuobWqogyDauWBVm",
                    "full_name": "Puja Dey",
                    "aadhaar_number": "246910235189",
                    "dob": "1996-09-09",
                    "gender": "F",
                    "address": {
                        "country": "India",
    ...
    type: any = ''
    isRenewal = false;
    disablefieldsidlist: any = ['Aadhaar_Card','Verify_Aadhaar','Resend_OTP_Aadhaar','REQUEST_FOR_OTP_Adhaar','Request_for_OTP_aadhar_no',
                                'Request_For_Otp_Mobile','Mobile_Number','Enter_Mobile_Otp','Verify_Otp_Mobile','Resend_Otp_Mobile',
                                ]
    isVerifiedCount = 0;                            
    constructor(private rest: RestApiService, private common: CommonService, private activeRoute: ActivatedRoute,
        ...
        private project:ProjectConfService) {
    ...
    getDistricts(fielddesc: string, data: any, dtl: any, flag: string, groupPosition: number = -1, index: number = -1) {
        console.log("fielddesc",fielddesc,"data",data)
        const dist = {
            stateid: data
        };
    ...
    getFormInfo() {
        ...
        this.rest.getProfileFormInfo(data).subscribe((res: any) => {
            this.common.loaderEnd();
            if (res.success) {
                console.log(JSON.stringify(res.signature))
                this.signatureImg = ''
                if (res.signature != null && res.signature != undefined && res.signature != '') {
                        ...
                        for (const [index, obj] of this.formDetails[step].entries()) {
                            if (obj.groupid !== null) {
                                    ...
                                    for (let i = 0; i < obj.fielddetailskeys[index].length; i++) {
                                        ...
                                        if (field[key][0].fielddesc == 'Aadhaar_Card') {
                                            if (field[key][0].isverified == 1) {
                                                ...
                                                this.isVerifiedCount = this.isVerifiedCount + 1
                                                this.hideOTPFieldBtns(index, 1, 'Aadhaar');
                                            }
                                        }
                                        ...
                                        if (field[key][0].fielddesc === "State" || field[key][0].fielddesc === "State_P" || field[key][0].fielddesc === "State_N"
                                            ...
                                            for (let i = 0; i < this.stateList.length; i++) {
                                                // console.log(field[key][0].fielddesc,"===",this.stateList[i].statename,field[key][0].fieldvalue)
                                                if (this.stateList[i].statename === field[key][0].fieldvalue) {
                                                    // console.log("field[key][0].fielddesc",field[key][0].fielddesc);
                                ...
                                if (obj.fielddetails.fielddesc === "State" || obj.fielddetails.fielddesc === "State_P"
                                    ...
                                    for (let i = 0; i < this.stateList.length; i++) {
                                        // console.log(obj.fielddetails.fielddesc,"===",this.stateList[i].statename,obj.fielddetails.fieldvalue)
                                        if (this.stateList[i].statename === obj.fielddetails.fieldvalue) {
                                            // console.log("obj.fielddetails.fielddesc",obj.fielddetails.fielddesc)
                    ...
                    for (const step of this.stepList) {
                        if (newObj[step]) {
                            for (const obj of newObj[step]) {
                                for (const [index,item] of this.backupDetails[step].entries()) {
                                    if (obj.parentfieldid === item.fielddetails.fieldid && this.common.checkParentFieldValue(obj.parentfieldvalue, item.fielddetails.fieldvalue, 1)) {
                                        if (obj.groupid !== null) {
                                            ...
                                            if (obj.fielddetails.fielddesc === 'State_C' || obj.fielddetails.fielddesc === 'State' || obj.fielddetails.fielddesc === 'State_P' || obj.fielddetails.fielddesc === 'State_N') {
                                                obj.fielddetails.controlvalue = this.stateName.join('|');
                                                for (let i = 0; i < this.stateList.length; i++) {
                                                    if (this.stateList[i].statename === obj.fielddetails.fieldvalue) {
                                                        this.getDistricts(obj.fielddetails.fielddesc, this.stateList[i].id, step, '', -1, index);
                ...
                if (this.isRenewal) {
                    setTimeout(()=>{ 
                        this.project.disableCalculationFieldList(this.disablefieldsidlist)
                    },2000)
    ...
    onRadioButtonChange(obj: any, step: string, parentItem: any = {}, flag = 'single', groupPosition = 0): void {
        ...
        if (flag === 'group') {
            ...
            console.log("parentItem.fielddesc",parentItem.fielddesc)
        ...
        if (Array.isArray(stepDtl)) {
            for (const obj of stepDtl) {
                if (!Array.isArray(obj.fielddetails)) {
                    if (obj.isrequired === 1 && !this.ignoreCheckType.includes(obj.fielddetails.controltype) && obj.fielddetails.fieldvalue === '') {
    ...
    confirmSubmit(): any {
        if (!this.isRenewal) {
            ...
            //         sessionStorage.setItem('issubmitted', '1');
    ...
    renewalSubmit() {
        ...
        this.common.loaderShow();
        this.rest.renewalProfile(data).subscribe((res: any) => {
            this.common.loaderEnd();
            this.closeModal();
            if (res.success) {
                // this.CreateuserCustomize();
            ...
            for (const obj of this.formDetails[step]) {
                if (obj.fielddetails.fielddesc === "Name_of_Proprietor") {
                    ...
                    if (!fieldValue.some(item => item.val === obj.fielddetails.fieldvalue)) {
                        fieldValue.push({
                            'id': obj.fielddetails.fieldid,
                            'val': obj.fielddetails.fieldvalue,
                            'tempid': obj.tempid,
                            'isuniqueid': obj.fielddetails.isuniqueid,
                            'fieldtype': obj.fielddetails.fieldtype,
                            'stepid': obj.stepid
                        })
                    }
                }
                ...
                if (obj.fielddetails.fielddesc === "State" || obj.fielddetails.fielddesc === "State_P"
                    ...
                    for (let i = 0; i < this.stateList.length; i++) {
                        if (this.stateList[i].statename === obj.fielddetails.fieldvalue) {
                            this.getDistricts(obj.fielddetails.fielddesc, this.stateList[i].id, step, '', grouppos)
                        }
            ...
            for (let i = 0; i < objData.fielddetailskeys[grouppos].length; i++) {
                ...
                if (objData.fielddetailskeys[grouppos][i] === "Name_" + this.entytyType1) {
                    ...
                    if (!fieldValue.some(item => item.val === objData.fielddetails[grouppos][objData.fielddetailskeys[grouppos][i]][0].fieldvalue)) {
                        fieldValue.push({
                            'id': objData.fielddetails[grouppos][objData.fielddetailskeys[grouppos][i]][0].fieldid,
                            'val': objData.fielddetails[grouppos][objData.fielddetailskeys[grouppos][i]][0].fieldvalue,
                            'tempid': objData.fielddetails[grouppos][objData.fielddetailskeys[grouppos][i]][0].tempid,
                            'isuniqueid': objData.fielddetails[grouppos][objData.fielddetailskeys[grouppos][i]][0].isuniqueid,
                            'fieldtype': objData.fielddetails[grouppos][objData.fielddetailskeys[grouppos][i]][0].fieldtype,
                ...
                if (objData.fielddetailskeys[grouppos][i] === "State" || objData.fielddetailskeys[grouppos][i] === "State_P"
                    || objData.fielddetailskeys[grouppos][i] === "State_N" || objData.fielddetailskeys[grouppos][i] === "State_C") {
                    ...
                    for (let i = 0; i < this.stateList.length; i++) {
                        if (this.stateList[i].statename === this.aadhaarData.address.state) {
                            this.getDistricts(objData.fielddetailskeys[grouppos][i], this.stateList[i].id, objData, 'group', grouppos)
                        }
    ...
    signatureUpload(event: any) {
        if (event.target.files.length > 0) {
            ...
            fd.append('userid', this.common.getUserId() + '');
            fd.append('file', event.target.files[0]);
            this.rest.uploadAgentSignature(fd).subscribe((res: any) => {
                if (res.success) {
    ...
    previewDetails() {
        ...
        this.router.navigate(['/pages/preview-details'])
    }

    disabledField(pos: number = -1) {
        for (let i = 0; i < this.disablefieldsidlist.length; i++) {
            setTimeout(() => {
                if (pos == -1) {
                        let element:any = document.getElementById(this.disablefieldsidlist[i])
                        if (element != null) {
                            element.disabled = true
                        }

                } else {
                        let element:any = document.getElementById(this.disablefieldsidlist[i] + pos)
                        if (element != null) {
                            element.disabled = true
                        }
                }
