...
export class AgentRegistrationComponent implements OnInit {
  ...
  getEntityTypeList(): void {
    this.rest.getEntityTypeByEntity({reraid: this.commonService.getReraId(), entityid: 2}).subscribe((res: any) => {
      if (res.success) {
  ...
  onPromotorTypeChange(event: any) {
    const position = event.target.selectedIndex - 1;
    this.EnterpriseName = this.entityTypeListTemp[position].entitytypedesc;
  ...
  show(type:any){
    if(type=='Enterprise'){
      ...
      this.selectedPromotorType ='';
      this.entityTypeListTemp= [];
      for(let arr of  this.entityTypeList){
        if(!arr.entitytypedesc.match(/Individual.*/)){
          this.entityTypeListTemp.push(arr)
        }
      }
    }else if(type=='Individual'){
      for(let arr of  this.entityTypeList){
        if(arr.entitytypedesc.match(/Individual.*/)){
          this.selectedPromotorType = arr.entitytypeid
        }
      }
