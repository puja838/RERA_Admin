        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                this.rest.getCaseByProject({projectRegNo: projectRegNo}).subscribe((res: any) => {
                    if (res.success) {
                        if (res.details.dataObj.length == 0) {
                            obj['Is there any Cases?'][0].fielddetails = [];
                            // obj['Is there any Cases?'][0].fielddetailskeys = [];
                            obj['Is_there_any_Cases'][0].fieldvalue = 'No';
                        } else {
                            obj['Is_there_any_Cases'][0].fieldvalue = 'Yes';
                            for (const caseObj of res.details.dataObj) {
                                const fieldDetails = JSON.parse(JSON.stringify(obj['Is there any Cases?'][0].fielddetails[0]))
                                delete fieldDetails['ADD_MORE_CASE'];
                                fieldDetails['Case_No'][0].fieldvalue = caseObj.CaseNo;
                                fieldDetails['Case_Description'][0].fieldvalue = caseObj.Descriptions;
                                fieldDetails['Case_Status'][0].fieldvalue = caseObj.Case_Status;
                                finalDetailsArr.push(fieldDetails);
                                // keys.splice(keys.indexOf('ADD_MORE_CASE'), 1);
                                finalKeyArr.push(['Case_No', 'Case_Description', 'Case_Status']);
                            }
    ...
    getPastProjects(step: any) {
        ...
        this.rest.getPastProjects(data).subscribe(async (res: any) => {
            if (res.success) {
                ...
                if (this.formDetails[step].length > 0) {
                    for (let t = 0; t < this.formDetails[step].length; t++) {
                        if (this.formDetails[step][t].groupid === 21) {
                            if (res.response.pastproject_values !== undefined) {
                                ...
                                if (res.response.pastproject_values.length > 0) {
                                    var storeFieldDetailsKeys = JSON.parse(JSON.stringify(this.formDetails[step][t].fielddetailskeys[0]));
                                    var defaultFieldKeysArray = [];
                                    var storeFieldDetails = JSON.parse(JSON.stringify(this.formDetails[step][t].fielddetails[0]));
                                    var defaultFieldDetails = JSON.parse(JSON.stringify(storeFieldDetails));
                                    var defaultFieldDetailsArray = [];
                                    var defaultLandDetailsValues = JSON.parse(JSON.stringify(storeFieldDetails["Land Details"][0].fielddetails[0]));
                                    var defaultLandDetailsValuesArray = JSON.parse(JSON.stringify(defaultLandDetailsValues));
                                    if (res.response.pastproject_values.length > 0) {
                                        this.formDetails[step][t].fielddetailskeys = [];
                                        this.formDetails[step][t].fielddetails = [];
                                        var field_values = res.response.pastproject_values;
                                        var field_keys = res.response.pastproject_keys;
                                        for (let i = 0; i < field_values.length; i++) {
                                            ...
                                            for (let j = 0; j < field_keys.length; j++) {
                                                if (field_keys[j] == "Land Details") {
                                                    var landdetails_values = JSON.parse(JSON.stringify(field_values[i]["Land Details"].landdetails_values));
                                                    var landdetails_keys = field_values[i]["Land Details"].landdetails_keys;
                                                    var landdetailsKeys = [];
                                                    var defaultLandDetailsArray = [];
                                                        ...
                                                        for (let p = 0; p < landdetails_values.length; p++) {
                                                            ...
                                                            for (let q = 0; q < landdetails_keys.length; q++) {
                                                                if (defaultLandDetailsValuesArray[landdetails_keys[q]] !== undefined) {
                                                                    if (defaultLandDetailsValuesArray[landdetails_keys[q]].length > 0) {
                                                                        if (landdetails_values[p][landdetails_keys[q]][0] !== undefined) {
                                                                            defaultLandDetailsValuesArray[landdetails_keys[q]][0].fieldvalue = landdetails_values[p][landdetails_keys[q]][0].projectfieldvalue;
                                                                        }
                                                                    }
                                                                }
                                                            }
                                        ...
                                        this.formDetails[step][t].fielddetailskeys = defaultFieldKeysArray;
                                        this.formDetails[step][t].fielddetails = defaultFieldDetailsArray;
                                    }
                                }
                            } else {
