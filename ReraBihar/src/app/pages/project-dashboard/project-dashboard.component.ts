...
export class ProjectDashboardComponent implements OnInit, OnDestroy {
  ...
  getProjectDashboardData(): void {
    ...
    this.rest.getProjectDashboardData(data).subscribe((res: any) => {
      if (res.success) {
        ...
        this.notificationCount = res.response.notificationCount;
      }
    });
  }
  ...
  goToExtension(projectId: number, projectuid: string) {
    this.router.navigate(['/pages/project-extension-dtl/' + projectId + '/' + projectuid])
  }
  ...
  listShow(flag: number, heading: string = '') {
    ...
    this.listType = flag;
    if (flag === 1) {
      this.linkName = 'Projects Registered';
      this.projectList = [];
      this.isRegister = true;
      this.getProjectList();
    } else if (flag === 2) {
      this.router.navigate(['/pages/queryDashboard'])
      /*this.linkName = 'Queries from RERA';
      if (this.pendingProjectList.length === 0) {
        this.fetchProjectDetailsForUser();
      }*/
    } else if (flag === 3) {
      this.linkName = 'Update Project details'
      this.updateProjectList = [];
      this.fetchUpdateProjectDetailsForUser(0);
    } else if (flag === 4) {
      this.linkName = 'Registered Applications';
      this.projectList = [];
      this.isRegister = false;
      this.getProjectList();
      
    } else if (flag === 5) {
      this.linkName = 'Upcoming / Pending QPR';
      this.updateProjectList = [];
      this.fetchUpdateProjectDetailsForUser(2);
    }else if (flag === 6) {
      this.linkName = 'Alerts & Notifications';
    }else if (flag === 7) {
      this.linkName = 'Project Extension';
      this.router.navigate(['/pages/project-extension'])
    }
  }
  ...
  projectListForExtension() {
    ...
    this.rest.projectListForExtension(data).subscribe((res: any) => {
      if (res.success) {
        ...
        this.extensionProjectList = res.response;
      }
    });
  }
